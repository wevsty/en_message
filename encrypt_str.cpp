...
std::string AES_ECB_encrypt(std::string sKey, std::string plainText)
{
    ...
    try {
    CryptoPP::AES::Encryption aes_encryption((byte *)key, CryptoPP::AES::MAX_KEYLENGTH);
...
std::string AES_ECB_decrypt(std::string sKey, std::string cipherText)
{
    ...
    try {
        CryptoPP::ECB_Mode<CryptoPP::AES>::Decryption ecbDecryption((byte *)key, CryptoPP::AES::MAX_KEYLENGTH);

        CryptoPP::HexDecoder decryptor(new CryptoPP::StreamTransformationFilter(ecbDecryption
...
std::string AES_CFB_encrypt(std::string sKey,std::string sIV, std::string plainText)
{
    ...
    CryptoPP::CFB_Mode_ExternalCipher::Encryption cfb_encryption(aes_encryption,iv);
...
std::string AES_CFB_decrypt(std::string sKey,std::string sIV, std::string cipherText)
{
    ...
    try {
        CryptoPP::CFB_Mode<CryptoPP::AES>::Decryption CFB_Decryption((byte *)key, CryptoPP::AES::MAX_KEYLENGTH,iv);

        CryptoPP::HexDecoder decryptor(new CryptoPP::StreamTransformationFilter(CFB_Decryption
                                                                                , new CryptoPP::StringSink(outstr)));
        decryptor.Put((byte *)cipherText.c_str(), cipherText.size());
        decryptor.MessageEnd();
    }
