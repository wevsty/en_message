...
#define ENMESSAGE_VERSION ("1.0.1")
...
{
    //QFileDialog
    QString file_path = QFileDialog::getOpenFileName(parent, QObject::tr("Open Password File"), ".", QObject::tr("Password Files(*.txt *.passwd)"));
...
{
    //QFileDialog
    QString file_path = QFileDialog::getSaveFileName(parent, QObject::tr("Save Password File"), ".", QObject::tr("Password Files(*.txt *.passwd)"));
    if(file_path.length() == 0)
    {
        return "";
    }
...
void MainWindow_UI::on_Button_load_clicked()
{
    ...
    pwd_file.close();
    QString qstr_password = QString::fromLocal8Bit(q_pwd_bytes.data(),q_pwd_bytes.size());
